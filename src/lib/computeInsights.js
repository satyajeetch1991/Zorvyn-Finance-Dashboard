...
export function getDashboardSummary(transactions) {
    const now = new Date();
...
export function generateInsights(transactions, fmt = formatCurrency) {
...
export function getCategoryMonthlyTrend(transactions, topN = 5) {
    const summaries = getMonthlySummaries(transactions);
    ...
    const series = topCategories.map((cat) => ({
        ...
        data: months.map((month) => transactions
...
export function getDailySpendingMap(transactions) {
    const map = {};
    for (const t of transactions) {
        if (t.type === "expense") {
