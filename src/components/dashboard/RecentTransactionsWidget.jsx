...
export function RecentTransactionsWidget() {
    ...
    return (<Card className="flex flex-col h-full">
      ...
      <CardContent className="flex-1 space-y-1">
        {recent.map((t, i) => (<motion.div key={t.id} initial={{ opacity: 0, x: -8 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.04 }} className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-muted/50 transition-colors">
