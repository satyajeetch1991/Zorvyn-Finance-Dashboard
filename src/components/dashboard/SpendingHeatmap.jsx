...
function getColor(amount, max) {
    if (amount === 0)
        return "var(--heatmap-empty, #f1f5f9)";
      ...
      <CardContent className="overflow-x-auto pb-4">
        <div className="relative inline-flex flex-col gap-1 min-w-max">
          ...
          <div className="flex" style={{ paddingLeft: LABEL_W }}>
            ...
            return (<div key={wi} style={{ width: CELL + GAP }} className="text-[10px] text-muted-foreground overflow-visible whitespace-nowrap">
                  {lbl ? lbl.label : ""}
                </div>);
        })}
          </div>
          ...
          <div className="flex gap-0">
            ...
            <div className="flex flex-col" style={{ width: LABEL_W, gap: GAP }}>
              {DAYS.map((d, i) => (<div key={d} style={{ height: CELL, lineHeight: `${CELL}px` }} className={`text-[10px] text-muted-foreground text-right pr-1.5 ${i % 2 === 0 ? "invisible" : ""}`}>
                  {d}
                </div>))}
            </div>
            {/* Cells — plain divs, no motion */}
            {grid.map((week, wi) => (<div key={wi} className="flex flex-col" style={{ gap: GAP }}>
                {week.map((cell, di) => {
          ...
          <div className="flex items-center gap-1.5 mt-2 pl-7">
            ...
            {["#f1f5f9", "#e0e7ff", "#a5b4fc", "#6366f1", "#4338ca"].map((c) => (<div key={c} style={{ width: CELL, height: CELL, borderRadius: 2, backgroundColor: c }}/>))}
            <span className="text-[10px] text-muted-foreground">More</span>
          </div>
        </div>

        {/* Tooltip */}
        {tooltip && (<div className="fixed z-50 pointer-events-none bg-popover border rounded-lg shadow-lg px-3 py-2 text-xs" style={{ left: tooltip.x + 12, top: tooltip.y - 40 }}>
