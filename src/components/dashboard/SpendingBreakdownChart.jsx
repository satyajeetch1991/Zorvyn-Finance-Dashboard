...
function CustomTooltip({ active, payload, code, locale }) {
    if (!active || !payload?.length)
        return null;
    const p = payload[0];
    return (<div className="rounded-lg border bg-card p-3 shadow-lg text-sm">
      ...
      <Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={top6} cx="50%" cy="50%" innerRadius="55%" outerRadius="80%" dataKey="total" nameKey="label" paddingAngle={3} onClick={handleClick} style={{ cursor: "pointer" }}>
                {top6.map((entry) => (<Cell key={entry.category} fill={CATEGORY_COLORS[entry.category] || "#6b7280"}/>))}
              </Pie>
              <Tooltip content={<CustomTooltip code={selectedCurrency} locale={currencyInfo.locale}/>}/>
              <Legend formatter={(value) => <span style={{ fontSize: 12 }}>{value}</span>} iconType="circle" iconSize={8}/>
            </PieChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
