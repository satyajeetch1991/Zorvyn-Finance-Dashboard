    ...
    return (<Card className="flex flex-col h-full">
      ...
      <CardContent className="flex-1 space-y-2">
        {upcoming.length === 0 ? (<div className="flex flex-col items-center justify-center h-full py-8 text-muted-foreground gap-2">
            ...
            return (<motion.div key={bill.id} initial={{ opacity: 0, x: -8 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className={`flex items-center justify-between p-2.5 rounded-lg ${isOverdue ? "bg-red-50 dark:bg-red-950/20" : isDueToday ? "bg-amber-50 dark:bg-amber-950/20" : "bg-muted/40"}`}>
