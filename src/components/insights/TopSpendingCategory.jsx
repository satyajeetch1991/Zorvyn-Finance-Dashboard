...
export function TopSpendingCategory() {
    ...
    const monthlyTrend = useMemo(() => {
        ...
        for (const t of transactions.filter((t) => t.type === "expense")) {
            if (!map[t.category])
                map[t.category] = { curr: 0, prev: 0 };
            if (curr && t.date.startsWith(curr))
                map[t.category].curr += t.amount;
            if (prev && t.date.startsWith(prev))
                map[t.category].prev += t.amount;
        }
        return map;
    ...
    return (<Card>
      ...
      <CardContent className="space-y-5">
        {top6.map((item, i) => {
            ...
            return (<motion.div key={item.category} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.4 }}>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {hasTrend && Math.abs(pct) >= 0.5 ? (<span className={cn("flex items-center gap-0.5 text-xs font-medium", pct > 0 ? "text-red-500" : "text-emerald-500")}>
                      {pct > 0 ? <TrendingUp className="h-3 w-3"/> : <TrendingDown className="h-3 w-3"/>}
                      {Math.abs(pct).toFixed(0)}%
                    </span>) : hasTrend ? (<span className="flex items-center gap-0.5 text-xs text-muted-foreground">
              ...
              <div className="h-2 rounded-full bg-muted overflow-hidden">
                <motion.div className="h-full rounded-full" style={{ backgroundColor: color }} initial={{ width: 0 }} whileInView={{ width: `${(item.total / maxTotal) * 100}%` }} viewport={{ once: true }} transition={{ delay: i * 0.08 + 0.2, duration: 0.7, ease: "easeOut" }}/>
              </div>
            </motion.div>);
        })}
      </CardContent>
