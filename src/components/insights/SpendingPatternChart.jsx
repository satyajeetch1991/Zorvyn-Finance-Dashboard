...
function CustomTooltip({ active, payload, label, code, locale, }) {
    ...
    if (nonZero.length === 0)
        return null;
    return (<div className="rounded-lg border bg-card p-3 shadow-lg text-sm min-w-[160px]">
      <p className="font-semibold mb-2 text-foreground">{label}</p>
      {nonZero.map((p) => (<div key={p.name} className="flex items-center justify-between gap-4 py-0.5">
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              ...
              <XAxis dataKey="label" tick={{ fill: textColor, fontSize: 11 }} tickLine={false} axisLine={false}/>
              ...
              {trend.series.map((s) => (<Line key={s.name} type="monotone" dataKey={s.name} stroke={s.color} strokeWidth={2} dot={{ r: 3, fill: s.color, strokeWidth: 0 }} activeDot={{ r: 5, strokeWidth: 0 }}/>))}
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
