...
function CustomTooltip({ active, payload, label, code, locale }) {
    ...
    return (<div className="rounded-lg border bg-card p-3 shadow-lg text-sm min-w-[180px]">
      <p className="font-semibold mb-2">{label}</p>
      {payload.map((p) => (<p key={p.name} className="flex justify-between gap-4 py-0.5" style={{ color: p.fill }}>
          <span>{p.name}</span>
...
function MomBadge({ current, previous }) {
    ...
    const up = pct > 0;
    return (<span className={cn("flex items-center gap-0.5 text-xs font-medium", up ? "text-red-500" : "text-emerald-500")}>
...
export function MonthlyComparison() {
    ...
    return (<motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }} className="space-y-4">
      {/* Bar chart */}
      <Card>
        <CardHeader>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {[...allMonths].reverse().map((m, i) => (<motion.tr key={m.month} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.04, duration: 0.3 }} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-2.5 px-4 text-right hidden sm:table-cell">
                      <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium", m.savingsRate > 20
