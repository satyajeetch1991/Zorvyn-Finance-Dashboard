...
export function MobileNav() {
    ...
    return (<nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 h-16 bg-card border-t border-border overflow-x-auto">
      <div className="flex h-full items-center min-w-max px-1">
        {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
            const isActive = href === "/dashboard" ? pathname === href : pathname.startsWith(href);
            return (<Link key={href} to={href} className={cn("flex flex-col items-center gap-0.5 px-3 py-2 text-[10px] font-medium transition-colors min-w-[60px]", isActive ? "text-primary" : "text-muted-foreground")}>
