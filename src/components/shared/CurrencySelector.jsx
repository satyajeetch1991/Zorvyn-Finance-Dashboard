    ...
    return (<DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        <div className="flex items-center justify-between px-2 py-1.5 border-b mb-1">
          ...
          {!isLoading && (<button onClick={(e) => { e.stopPropagation(); fetchRates(); }} className="text-[10px] text-primary hover:underline">
              Refresh
            </button>)}
        </div>
        ...
        {Object.keys(CURRENCIES).map((code) => {
            ...
            return (<DropdownMenuItem key={code} onClick={() => setCurrency(code)} className={`flex items-center gap-2 text-sm cursor-pointer ${selectedCurrency === code ? "font-semibold bg-accent" : ""}`}>
