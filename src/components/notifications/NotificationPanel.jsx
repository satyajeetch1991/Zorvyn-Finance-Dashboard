...
function NotificationItem({ n }) {
    ...
    return (<div className={`relative flex gap-3 rounded-lg p-3 transition-colors ${n.isRead ? "opacity-60" : cfg.bg} ${!n.isRead ? "cursor-pointer" : ""}`} onClick={() => !n.isRead && markRead(n.id)}>
      ...
      <button className="shrink-0 mt-0.5 text-muted-foreground hover:text-foreground transition-colors" onClick={(e) => { e.stopPropagation(); dismiss(n.id); }}>
...
export function NotificationPanel({ open, onClose }) {
    ...
    return createPortal(<AnimatePresence>
      {open && (<>
          ...
          <motion.div className="fixed inset-0 bg-black/30 backdrop-blur-sm" style={{ zIndex: 100 }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}/>

          {/* Panel — z-[101] to sit above backdrop */}
          <motion.div className="fixed top-0 right-0 h-full w-full max-w-sm bg-background border-l shadow-2xl flex flex-col" style={{ zIndex: 101 }} initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 280 }}>
            {/* Header */}
            <div className="flex items-center justify-between border-b px-4 py-3 shrink-0">
              <div className="flex items-center gap-2">
                <Bell className="h-4 w-4 text-muted-foreground"/>
                <span className="font-semibold text-sm">Notifications</span>
                {notifications.filter((n) => !n.isRead).length > 0 && (<span className="rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-bold text-white">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {notifications.length === 0 ? (<div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground">
                  <Bell className="h-10 w-10 opacity-20"/>
                  <p className="text-sm">No notifications</p>
                </div>) : (<>
                  {today.length > 0 && (<div className="space-y-2">
    ...
    </AnimatePresence>, document.body // ← renders outside any stacking context
    );
}
